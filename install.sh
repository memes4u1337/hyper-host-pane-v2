...
if ss -tulpn 2>/dev/null | grep -q ':8083'; then
  echo ""
  echo "⚠️  ВНИМАНИЕ: порт 8083 занят — похоже, HestiaCP всё ещё запущена."
  echo "   Сначала выполни: sudo bash uninstall_hestia.sh"
  echo "   Продолжаю через 5 секунд, но рекомендую прервать (Ctrl+C) и сначала удалить Hestia..."
  sleep 5
fi
...
echo ""
export HH_ADMIN_USER HH_ADMIN_PASS
"$INSTALL_DIR/venv/bin/python3" - "$INSTALL_DIR" <<'PYEOF'
import sys, os, sqlite3
from werkzeug.security import generate_password_hash

install_dir = sys.argv[1]
username = os.environ["HH_ADMIN_USER"]
password = os.environ["HH_ADMIN_PASS"]

db = sqlite3.connect(os.path.join(install_dir, "app", "hyperhost.db"))
db.execute(
...
db.commit()
db.close()
print("Учётка администратора создана:", username)
PYEOF
unset HH_ADMIN_PASS
